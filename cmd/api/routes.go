...
func (app *application) routes() *httprouter.Router {
	...
	router.HandlerFunc(http.MethodGet, "/v1/invoices/:id", app.invoiceHandler)
	...
	router.HandlerFunc(http.MethodGet, "/v1/menus/:id", app.menuHandler)

	router.HandlerFunc(http.MethodPost, "/v1/menus", app.createMenuHandler)

	router.HandlerFunc(http.MethodPatch, "/v1/menu/:id", app.updateMenuHandler)

	router.HandlerFunc(http.MethodGet, "/v1/orders", app.ordersHandler)

	router.HandlerFunc(http.MethodGet, "/v1/orders/:id", app.orderHandler)
	...
	router.HandlerFunc(http.MethodGet, "/v1/tables/:id", app.tableHandler)

	router.HandlerFunc(http.MethodPost, "/v1/tables", app.createTableHandler)

	router.HandlerFunc(http.MethodPatch, "/v1/tables/:id", app.updateTableHandler)

	router.HandlerFunc(http.MethodGet, "/v1/orderItems", app.orderItemsHandler)

	router.HandlerFunc(http.MethodGet, "/v1/orderItems/:id", app.orderItemHandler)

	router.HandlerFunc(http.MethodGet, "/v1/orderItems-order/:id", app.orderItemsByOrderHandler)

	router.HandlerFunc(http.MethodPost, "/v1/orderItems", app.createOrderItemHandler)

	router.HandlerFunc(http.MethodPatch, "/v1/orderItems/:id", app.updateOrderItemHandler)
